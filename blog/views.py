...
def index(request):
    ...
    ).annotate(total_comments=Count('related_comments'))
    popular_posts_with_prefetch = \
        popular_posts_with_comments.prefetch_related('author')

    most_fresh_posts = Post.objects.annotate(
        total_comments=Count('related_comments', distinct=True)
    ).order_by('-published_at')[:5]
    fresh_posts_with_prefetch = \
    ...
    context = {
        'most_popular_posts': [
            serialize_post_optimized(post) for post in popular_posts_with_prefetch
        ],
        'page_posts': [
            serialize_post_optimized(post) for post in fresh_posts_with_prefetch
        ],
